...
**Motivation (human needs) → Identity (archetype) → Perception (visual cognition + design) → Behavior (persuasion) → Deployment (distribution channels) → Opportunity (jobs/clients/speaking)**.
...
For *students/founders*, your “one archetype” is best reframed as:
...
```mermaid
flowchart TD
  A[Motivation map: human needs] --> B[Identity: primary archetype]
  B --> C[Expression: design primitives]
  C --> D[Persuasion: embedded influence structure]
  D --> E[Deployment: portfolio + publishing + events]
  E --> F[Opportunities: interviews, clients, talks]
  C --> G[Attention gate: stop-scroll + scanability]
  G --> D
  B --> H[Coherence checks: voice, visuals, claims]
  H --> C
```
...
```mermaid
graph LR
  Person[Individual / Founder] --> Identity[Authentic identity]
  Identity --> Archetype[Primary archetype]
  Archetype --> Design[Design primitives]
  Archetype --> Narrative[Story + voice]
  Needs[Needs model] --> Narrative
  Design --> Perception[Attention + trust]
  Persuasion[Influence principles] --> CTA[Commitments + actions]
  Perception --> CTA
  Narrative --> Proof[Proof-of-work artifacts]
  Proof --> Channels[LinkedIn / site / meetups]
  Channels --> Opportunities[Jobs / clients / speaking]
  Product[Product positioning] --> Design
  Product --> Narrative
```
